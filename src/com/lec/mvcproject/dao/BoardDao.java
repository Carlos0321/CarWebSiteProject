...
public class BoardDao {
	...
	public BoardDto contentView(int bno) {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = "SELECT B.*, MNAME FROM BOARD B, MEMBER M" + "    WHERE M.MID=B.MID AND BNO=?";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bno);
	...
	// (9)답변글 쓰기
	public int replyBoard(String mid, String bsubject, String bcontent, String bfile, String bip, int bgroup, int bstep,
			int bindent) {
